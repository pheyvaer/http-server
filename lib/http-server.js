...
function HttpServer(options) {
  ...
  if (this.conneg) {
    before.push(function (req, res) {
      ...
      var supportedMimeTypes = self.existingTypesForFile(req.url);
      var selectedType = accept.type(supportedMimeTypes);

      if (selectedType) {
        res.setHeader('Content-Type', selectedType);
        req.url += '.' + typeToExtension[selectedType];
      }
...
HttpServer.prototype.existingTypesForFile = function (url) {
  var fullPath = path.join(this.root, url);

  if (!this.availableTypes[url]) {
    var mimeTypes = Object.keys(typeToExtension);
    var supportedMimeTypes = [];

    mimeTypes.forEach(function (type) {
      var extension = typeToExtension[type];
    ...
    this.availableTypes[url] = supportedMimeTypes;
  }

  return this.availableTypes[url];
};

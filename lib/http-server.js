  ...
  if (this.conneg) {
    before.push(function (req, res) {
      if (req.url.indexOf('.') === -1) {
        ...
        self.existingTypesForFile(req.url, acceptTypes).then(function (supportedMimeTypes) {
          var selectedType = accept.type(Object.keys(supportedMimeTypes));

          if (!selectedType && acceptTypes.indexOf('*/*') !== -1 && Object.keys(supportedMimeTypes).length > 0) {
      ...
      files.forEach(function (file) {
        if (file.indexOf('.') !== -1) {
          ...
          if (filename === first) {
            ...
            if (!type) {
              // hard code n-triples and n-quads (I'm working on a PR for mini-db (used by mime-types))
              switch (extension) {
                case '.nt':
